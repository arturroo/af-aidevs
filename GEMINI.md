...
- **Cloud Run Task Microservice Standards:** Every Cloud Run microservice implementing a lesson task MUST provide:
  ...
  - **Testing Private Cloud Run Endpoints via curl (PowerShell):** Microservices are deployed privately (`public = false`), requiring an OIDC Identity Token. In Windows PowerShell, use `curl.exe` to avoid conflicts with PowerShell's built-in `curl` alias (`Invoke-WebRequest`):
    ...
    - **Run Task Execution (`POST /run`):**
      ...
      ```
  - **Mandatory Container Scaffolding Standards:** To prevent **Cross-Platform Venv Poisoning** (where a local Windows `.venv` with `.exe` binaries is copied into a Linux container via `COPY . .`, causing Cloud Run `Error code 9` on `PORT=8080`), eliminate multi-gigabyte upload bottlenecks in `gcloud builds submit`, and prevent **Local Disk Poisoning** (where stale workspace files get baked into production images), EVERY Cloud Run service folder MUST include from day one:
    - `.dockerignore`: Strictly ignoring `.git`, `.venv`, `__pycache__`, `*.pyc`, `.env`, `.env.*`, `tests`, `workspace`, and `run_notes.txt`.
    - `.gcloudignore`: Strictly ignoring `.gcloudignore`, `.git`, `.gitignore`, `.venv`, `__pycache__`, `*.pyc`, `.env`, `.env.*`, `tests`, `workspace`, and `run_notes.txt`.
    - **Strict Container Statelessness & Zero Disk Poisoning:** Cloud Run containers MUST remain 100% stateless and hermetic. NEVER write intermediate agent scratch buffers or staging files to local container disk storage (`workspace/`), as local files get accidentally baked into Docker images via `COPY . .` causing stale data poisoning and false validation failures. Agents MUST interact with workspaces exclusively via remote session-isolated MCP services (`cr-mcp-workspace`) using `X-Session-ID`. All `.dockerignore`, `.gcloudignore`, and `.gitignore` files MUST strictly exclude `workspace/` and `**/workspace/`.
...
- **The "Local `run_notes.txt` in Git Repo" Antipattern:** Writing, staging, or committing `run_notes.txt` to the Git repository is a strict architectural and security antipattern. Runtime execution notes containing unanonymized flags (`{FLG:...}`) and execution traces MUST ONLY be persisted in private GCS workspaces (`gs://af-aidevs-workspaces/sa-<service-name>/<session_id>/run_notes.txt`). The Git repository must remain 100% free of `run_notes.txt` (strictly enforced via `.gitignore` patterns `run_notes.txt` and `**/run_notes.txt`).
- **Granular Entity-Level Observability in Deterministic Tools & Code:** Deterministic pipeline tools (staging, validation gates, batch upload, extraction, transliteration) MUST log individual entity progress and status with explicit counters (e.g. `Validation [PASS] [1/29] for '/miasta/opalino'`, `Staging file [1/29] into cr-mcp-workspace: ...`, `Batch payload item [1/29]: createFile -> ...`). When any validation or contract check fails, it MUST log a `WARNING` with the exact violation rule, path, and reason. This is a foundational observability requirement ensuring engineers and SREs can isolate pipeline failures in Cloud Logging within seconds without guesswork.

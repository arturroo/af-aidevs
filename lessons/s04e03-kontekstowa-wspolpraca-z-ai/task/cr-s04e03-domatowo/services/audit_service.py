"""Real-time BigQuery telemetry and audit logging service for S04E03."""
...
def generate_session_id(backend: str = "langchain") -> str:
    """Generates standardized session ID: s04e03_{backend}_{YYYYMMDD_HHMMSS} (Europe/Zurich)."""
    now = datetime.now(ZURICH_TZ).strftime("%Y%m%d_%H%M%S")
    return f"s04e03_{backend}_{now}"
...
class AuditService:
    ...
    async def log_event(
        ...
        # Safely insert into BigQuery in background executor to avoid blocking event loop

"""Real-time BigQuery telemetry and audit logging service for S04E02."""
...
def generate_session_id(backend: str = "langchain") -> str:
    """Generates standardized session ID: s04e02_{backend}_{YYYYMMDD_HHMMSS} (Europe/Zurich)."""
    now = datetime.now(ZURICH_TZ).strftime("%Y%m%d_%H%M%S")
    return f"s04e02_{backend}_{now}"

"""Real-time BigQuery telemetry and audit logging service for S03E05."""
...
def generate_session_id(backend: str = "langchain") -> str:
    """Generates standardized session ID: s03e05_{backend}_{YYYYMMDD_HHMMSS} (Europe/Zurich)."""
    now = datetime.now(ZURICH_TZ).strftime("%Y%m%d_%H%M%S")
    return f"s03e05_{backend}_{now}"
    ...
    async def log_event(
        ...
        def _insert():
            ...
            try:
                ...
                logger.debug(f"Direct BigQuery logging failed, falling back to stdout: {e}")
                print(f"[AUDIT-FALLBACK] {timestamp} | {session_id} | {actor} | {step_type}: {content[:200]}")

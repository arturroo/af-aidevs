...
### 2.2. `cr-mcp-workspace` (Globalny Serwer Przestrzeni Roboczej)
- **Rola:** Zarządzanie pełną przestrzenią roboczą agenta poprzez narzędzia takie jak `list_files`, `read_file`, `write_file`. Umożliwia wczytywanie całych plików do analizy przez agenta, bez konieczności ich ponownego generowania.
...
### 2.3. `cr-mcp-web` (Globalny Serwer Web Fetcher) - *Opcjonalny / Do rozważenia*
- **Rola:** Pobieranie treści ze stron www z ominięciem przepychania dużych payloadów przez głównego agenta.
- **Infrastruktura:** Cloud Run (zamiast Cloud Functions, ze względu na lepsze wsparcie dla `fastmcp`). Posiada wyjście do internetu (egress). Zamontowany ten sam bucket FUSE co `cr-mcp-workspace`.
- **Zagrożenie SSRF (Server-Side Request Forgery):** Wystawienie generycznych narzędzi `get` lub `post` pozwala agentowi (a potencjalnie atakującemu przez prompt injection) na pobieranie dowolnych zasobów z sieci, co może prowadzić do wykonania szkodliwego kodu (exploitów) lub wyciągania danych. Jeśli zadanie z lekcji tego bezpośrednio nie wymaga, wdrożenie tego serwera powinno zostać odłożone. Autoryzowane zapytania do wytyczonych API (jak AIDEVS) lepiej realizować jako lokalne narzędzia agenta.
- **Działanie (jeśli wdrożone):** Narzędzie pobiera stronę internetową, zapisuje wynik bezpośrednio w buckecie GCS w katalogu wywołującego agenta i zwraca relatywną ścieżkę do pliku (np. `/web/1234.html`). Agent może potem przekazać tę ścieżkę do `cr-mcp-workspace`.
...
- **Model:** Używa szybkiego, taniego modelu klasy "Flash-Lite" (np. `gemini-3.1-flash-lite-preview`).
...
- **Rola `run.invoker`:** Terraform musi nadać rolę Cloud Run Invoker:
  - `sa-agent-s01e05` może wywoływać `cr-mcp-workspace`, `cr-mcp-web` oraz `cr-model-armor`.
- **Uprawnienia Storage:** `sa-mcp-workspace` oraz `sa-mcp-web` muszą posiadać `Storage Object Admin` dla bucketa `af-aidevs-workspaces`.
...
3. **Pętla Agenta:** (Zależnie od implementacji) LangChain wywołuje narzędzie podłączone do `cr-mcp-web`, wysyłając URL. W nagłówkach HTTP leci Identity Token OIDC konta `sa-agent-s01e05`.
4. **Zapis do GCS:** `cr-mcp-web` pobiera stronę, zapisuje ją do `gs://af-aidevs-workspaces/sa-agent-s01e05/web/logs.txt`.
5. **Wynik narzędzia:** Agent otrzymuje odpowiedź `{"status": "success", "file": "/web/logs.txt"}`.
6. **Analiza:** Agent potrzebuje przeanalizować zawartość pliku. Wywołuje narzędzie `read_file` z serwera `cr-mcp-workspace`, podając ścieżkę `/web/logs.txt`.
7. **Izolowany Odczyt:** `cr-mcp-workspace` dekoduje JWT agenta, buduje pełną ścieżkę do FUSE: `/mnt/workspaces/sa-agent-s01e05/web/logs.txt`, czyta cały plik i zwraca go do agenta.
...
3. Implementacja globalnych mikroserwisów `cr-mcp-workspace`, `cr-mcp-web` (opcjonalnie), `cr-model-armor`.

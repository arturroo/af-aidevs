...
AIDEVS_VERIFY_URL = raw_verify.replace("/api/verify", "/verify") if raw_verify.endswith("/api/verify") else raw_verify

AIDEVS_FAILURE_DATA_URL = (
    os.getenv("AIDEVS_FAILURE_DATA_URL")
    or (f"https://hub.ag3nts.org/data/{AIDEVS_API_KEY}/failure.log" if AIDEVS_API_KEY else "")
)
...
# BigQuery Auditing
BQ_DATASET = os.getenv("BQ_DATASET") or "s02e03"

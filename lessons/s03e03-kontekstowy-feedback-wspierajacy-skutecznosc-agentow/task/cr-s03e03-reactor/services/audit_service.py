...
def generate_session_id(backend: str = "langchain") -> str:
    """Generates standardized session ID: s03e03_{backend}_{YYYYMMDD_HHMMSS} (Europe/Zurich)."""
    now = datetime.now(ZURICH_TZ).strftime("%Y%m%d_%H%M%S")
    return f"s03e03_{backend}_{now}"
    ...
    async def log_event(
        ...
        def _insert():
            ...
            try:
                ...
                if errors:
                    logger.error(
                        f"[BigQuery Audit Error] {self.full_table_id}: {errors}"
                    )
